...
if len(st.session_state["ds"]) == 1000:
    st.warning(
        "Attention only a subset of size 1000 of the modified / filtered examples can be shown in this application"
    )
...
if "_filter_" in option_check:
    ...
    col_title.markdown(
        f"<h1 style='text-align: center'>Example n°{st.session_state['idx']}</h1>",
        unsafe_allow_html=True,
    )
    col_button_next.button(
        "Go to next example",
